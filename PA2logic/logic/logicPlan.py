...
def plTrueInverse(assignments: Dict[Expr, bool], inverse_statement: Expr) -> bool:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # util.raiseNotDefined()
    if pl_true(inverse_statement, assignments) == True:
        return False
    else:
        return True
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    # util.raiseNotDefined()
    disj_list = []
    for item in itertools.combinations(literals, 2):
        disj_list.append(~item[0] | ~item[1])
    return conjoin(disj_list)
    "*** END YOUR CODE HERE ***"
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    for coord in all_coords:
        Expr_list.append(PropSymbolExpr(wall_str, coord[0], coord[1]) >> ~PropSymbolExpr(pacman_str, coord[0], coord[1], time=t))
    pacphysics_sentences.append(conjoin(Expr_list))
    
    non_outer_wall_Expr_list = []
    for non_outer_wall_coord in non_outer_wall_coords:
        non_outer_wall_Expr_list.append(PropSymbolExpr(pacman_str, non_outer_wall_coord[0], non_outer_wall_coord[1], time=t))
    pacphysics_sentences.append(exactlyOne(non_outer_wall_Expr_list))
    
    action_Expr_list = []
    for action in DIRECTIONS:
        action_Expr_list.append(PropSymbolExpr(action, time=t))
    pacphysics_sentences.append(exactlyOne(action_Expr_list))
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(0, 50): 
        ...
        for non_wall_coord in non_wall_coords:
            non_wall_Expr_list.append(PropSymbolExpr(pacman_str, non_wall_coord[0], non_wall_coord[1], time=t))
        KB.append(exactlyOne(non_wall_Expr_list))
        ...
        if model != False:
            ...
            for action in DIRECTIONS:
                action_Expr_list.append(PropSymbolExpr(action, time=t))
            KB.append(exactlyOne(action_Expr_list))

            for non_wall_coord in non_wall_coords:
                KB.append(pacmanSuccessorAxiomSingle(non_wall_coord[0], non_wall_coord[1], t + 1, walls_grid))
...
def foodLogicPlan(problem) -> List:
    ...
    food_Expr_list = []
    for food in foods:
        food_Expr_list.append(PropSymbolExpr(food_str, food[0], food[1], time=0))
    KB.append(conjoin(food_Expr_list))
    ...
    for t in range(0, 50): 
        ...
        food_Expr_list = []
        for food in foods:
            food_Expr_list.append(PropSymbolExpr(food_str, food[0], food[1], time=t))
        assertion = ~disjoin(food_Expr_list)
        ...
        if model != False:
            ...
            for non_wall_coord in non_wall_coords:
                KB.append(pacmanSuccessorAxiomSingle(non_wall_coord[0], non_wall_coord[1], t + 1, walls))

            for food in foods:
                KB.append(PropSymbolExpr(food_str, food[0], food[1], time=t+1) % (PropSymbolExpr(food_str, food[0], food[1], time=t) & (~PropSymbolExpr(pacman_str, food[0], food[1], time=t))))
                KB.append(~PropSymbolExpr(food_str, food[0], food[1], time=t+1) % (~PropSymbolExpr(food_str, food[0], food[1], time=t) | (PropSymbolExpr(pacman_str, food[0], food[1], time=t))))
    ...
    for t in range(agent.num_timesteps):
        ...
        for non_outer_wall_coord in non_outer_wall_coords:
            if entails(conjoin(KB), PropSymbolExpr(pacman_str, non_outer_wall_coord[0], non_outer_wall_coord[1], time=t)):
                KB.append(PropSymbolExpr(pacman_str, non_outer_wall_coord[0], non_outer_wall_coord[1], time=t))
            if entails(conjoin(KB), ~PropSymbolExpr(pacman_str, non_outer_wall_coord[0], non_outer_wall_coord[1], time=t)):
                KB.append(~PropSymbolExpr(pacman_str, non_outer_wall_coord[0], non_outer_wall_coord[1], time=t))
            else:
    ...
    for t in range(agent.num_timesteps):
        ...
        for non_outer_wall_coord in non_outer_wall_coords:
            if entails(conjoin(KB), PropSymbolExpr(wall_str, non_outer_wall_coord[0], non_outer_wall_coord[1])):
                KB.append(PropSymbolExpr(wall_str, non_outer_wall_coord[0], non_outer_wall_coord[1]))
                known_map[non_outer_wall_coord[0]][non_outer_wall_coord[1]] = 1
            if entails(conjoin(KB), (~PropSymbolExpr(wall_str, non_outer_wall_coord[0], non_outer_wall_coord[1]))):
                KB.append(~PropSymbolExpr(wall_str, non_outer_wall_coord[0], non_outer_wall_coord[1]))
                known_map[non_outer_wall_coord[0]][non_outer_wall_coord[1]] = 0
        agent.moveToNextState(agent.actions[t])
        "*** END YOUR CODE HERE ***"
        yield known_map
