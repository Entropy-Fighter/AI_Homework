...
def constructBayesNet(gameState):
    ...
    for h_pos in gameState.getPossibleHouses():
        for o_pos in gameState.getHouseWalls(h_pos):    
            o_v = OBS_VAR_TEMPLATE % o_pos
            obsVars.append(o_v)
            variableDomainsDict[o_v] = OBS_VALS
            edges.append((GHOST_HOUSE_VAR, o_v))
            edges.append((FOOD_HOUSE_VAR, o_v))    
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    # util.raiseNotDefined()
    p = 0
    for assignment in inference.inferenceByVariableElimination(bayesNet, FOOD_HOUSE_VAR, evidence, eliminationOrder).getAllPossibleAssignmentDicts():
        p1 = inference.inferenceByVariableElimination(bayesNet, FOOD_HOUSE_VAR, evidence, eliminationOrder).getProbability(assignment)
        if p1 > p:
            res = assignment
            p = p1
    return res
